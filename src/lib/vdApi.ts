...
export function buildVariablesApi(activeEnvKey?: string): {
  ...
  return {
    get: async (key: string): Promise<any> => {
      try {
        const ipcValue = await (typeof window !== 'undefined' ? (window as any) : undefined)?.electron?.variables?.get?.(key);
        if (ipcValue !== undefined) return ipcValue;

        const state = await (typeof window !== 'undefined' ? (window as any) : undefined)?.electron?.state?.get();
        const projectPath = state?.activeDirectory || "";
        const fileContent = await (typeof window !== 'undefined' ? (window as any) : undefined)?.electron?.files?.read(
    ...
    set: async (key: string, value: any): Promise<void> => {
      try {
        const electron = (typeof window !== 'undefined' ? (window as any) : undefined)?.electron;
        // Use provided activeEnvKey, or fetch it if not provided
        const envKey =
          activeEnvKey ??
          (await electron?.variables?.getActiveEnvKey?.());
        console.log("[vdApi.variables.set]", {
          key,
          envKey,
          activeEnvKeyProvided: Boolean(activeEnvKey),
        });
        await electron?.variables?.mergeVariables?.(
          { [key]: value },
          envKey,
        );
        if (envKey && envKey !== "__global__") {
          await electron?.variables?.deleteKey?.(
            key,
            "__global__",
          );
        }
...
export function buildEnvApi(): { get: (key: string) => Promise<any> } {
  return {
    get: async (key: string): Promise<any> => {
      try {
        const envLoad = await (typeof window !== 'undefined' ? (window as any) : undefined)?.electron?.env?.load?.();
